...
func newFakeProxy(c *api.Config) *fakeProxy {
	...
	for i, c := range requests {
		...
		if len(c.ExpectedCookies) > 0 {
			...
			for _, x := range c.ExpectedCookies {
				assert.NotNil(t, utils.FindCookie(x, resp.Cookies()), "case %d, expected cookie %s not found", i, x)
			}
		}
		if c.OnResponse != nil {
...
func setRequestAuthentication(cfg *api.Config, client *resty.Client, request *resty.Request, c *fakeRequest, token string) {
...
func TestMetricsMiddleware(t *testing.T) {
	...
	requests := []fakeRequest{
		{
			URI:                     constants.OauthURL + constants.MetricsURL,
...
func TestCustomHeaders(t *testing.T) {
	...
	for _, c := range requests {
		cfg := newFakeKeycloakConfig()
		cfg.Resources = []*api.Resource{{URI: "/admin*", Methods: constants.AllHTTPMethods}}
