...
func (r *oauthProxy) proxyMiddleware(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if utils.IsUpgradedConnection(req) {
			...
			if err := utils.TryUpdateConnection(req, w, r.endpoint); err != nil {
		...
		case nil:
			...
			switch req.TLS {
			case nil:
				req.URL.Scheme = constants.HTTPSchema
			default:
				req.URL.Scheme = constants.HTTPSSchema
			}
		default:
		...
		req.Header.Add("X-Forwarded-For", utils.RealIP(req))
...
func (r *oauthProxy) forwardProxyHandler() func(*http.Request, *http.Response) {
	...
	go func() {
		for {
			...
			if state.wait {
				...
				duration := utils.GetWithin(state.expiration, 0.85)
	...
	return func(req *http.Request, resp *http.Response) {
		...
		if len(r.config.ForwardingDomains) == 0 || utils.ContainsSubString(hostname, r.config.ForwardingDomains) {
			req.Header.Set("X-Forwarded-Agent", constants.Prog)
